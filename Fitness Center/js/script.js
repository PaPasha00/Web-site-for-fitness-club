$(document).ready(function(){
  ...
  document.querySelectorAll('.menuButton').forEach(function(element){
	  element.onclick = displayed;
  });

  document.querySelectorAll('.puncts').forEach(function(element) {
    element.onclick = displayed;  
  });
  ...
  function displayed(){
    let elem = document.getElementById('MenuScreen');

    if (elem.classList.contains("disp")) {
...
document.addEventListener('DOMContentLoaded', function () {
  // конечная дата
  const deadline = new Date(2023, 03, 12);
  // id таймера
  let timerId = null;
